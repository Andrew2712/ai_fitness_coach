...
def coach(user_id: int):
    ...
    from agents.analysis_agent import build_profile, analyze_trends, calculate_recovery_index, calculate_progress_score
...
def recovery_plan(user_id: int):
    import pandas as pd
    from controller import DATA_PATH, FEATURE_PATH
    from agents.analysis_agent import build_profile, analyze_trends, calculate_recovery_index, generate_recovery_plan
    df = pd.read_csv(DATA_PATH, parse_dates=["Date"])
    with open(FEATURE_PATH) as f:
        FEATURES = json.load(f)
    user_data = df[df["Id"] == user_id]
    ...
    return {
        ...
        "note":           fatigue_notes.get(fatigue, "Stay hydrated and listen to your body.")
    }
...
def register(req: RegisterRequest, db: Session = Depends(get_db)):
    ...
    user = DBUser(email=req.email, username=req.username, hashed_password=hash_password(req.password))
    db.add(user)
    db.commit()
    db.refresh(user)
...
def login(req: LoginRequest, db: Session = Depends(get_db)):
    ...
    return {
        "token": token,
        "user": {"id": user.id, "email": user.email, "username": user.username, "fitbit_connected": bool(user.fitbit_user_id)}
    }
...
def fitbit_login_user(dashboard_user_id: str):
    ...
    redirect = "https://aifitnesscoach-production-373b.up.railway.app/fitbit/callback"
    url = (
