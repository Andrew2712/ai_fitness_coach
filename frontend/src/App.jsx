...
function SparkBar({ data, color, h = 44, labels }) {
  ...
  return (
    <svg viewBox={`0 0 ${tw} ${h+(labels?14:0)}`} width="100%" height={h+(labels?14:0)} preserveAspectRatio="none">
      {vd.map((v,i)=>{
        const bh=Math.max(3,(v/mx)*(h-4)), x=i*(bw+gap);
        return (<g key={i}>
...
function StatusBar({ value }) {
  const segs=[
    {label:"Detraining",color:"#94a3b8"},{label:"Recovery",color:T.blue},
    {label:"Productive",color:T.green},{label:"Peaking",color:T.amber},{label:"Overreaching",color:T.red}
  ];
  return (
    <div>
...
function Card({children,style:extra,accent}){
  return <div style={{background:T.card,borderRadius:12,border:`1px solid ${T.border}`,borderTop:accent?`3px solid ${accent}`:`1px solid ${T.border}`,padding:"16px",boxShadow:"0 1px 4px rgba(0,0,0,0.06)",...extra}}>{children}</div>;
}
function SectionLabel({children}){return <div style={{fontSize:11,fontWeight:700,color:T.muted,letterSpacing:1,textTransform:"uppercase",marginBottom:12,fontFamily:"system-ui"}}>{children}</div>;}
function MetricLabel({children}){return <div style={{fontSize:11,color:T.muted,fontFamily:"system-ui",marginBottom:2}}>{children}</div>;}
      ...
      <Card accent={T.border} style={{marginBottom:isMobile?16:24}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          ...
          <button onClick={()=>window.location.href=`${API_URL}/strava/login/${authUser.id}`}
            ...
            <StravaIcon size={14} color="#fff"/>Connect Strava
          </button>
        </div>
      </Card>
    );
  ...
  if (!d) return (
    <Card accent={T.strava} style={{marginBottom:isMobile?16:24}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <button onClick={onSync} style={{background:T.stravaLight,border:`1px solid ${T.strava}`,borderRadius:8,padding:"8px 14px",color:T.strava,fontSize:12,fontWeight:600,cursor:"pointer"}}>{stravaSyncing?"Syncing…":"🔄 Load Data"}</button>
      </div>
    </Card>
  );
    ...
    <Card accent={T.strava} style={{marginBottom:isMobile?16:24}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,flexWrap:"wrap",gap:8}}>
        <div>
          ...
          <div style={{fontSize:11,color:T.muted}}>{d.city}{d.city&&d.country?", ":""}{d.country} • {d.followers} followers • {d.following} following</div>
        </div>
        <div style={{display:"flex",gap:6}}>
          <button onClick={onSync} style={{background:T.stravaLight,border:`1px solid ${T.strava}`,borderRadius:8,padding:"6px 10px",color:T.strava,fontSize:12,fontWeight:600,cursor:"pointer"}}>{stravaSyncing?"…":"🔄"}</button>
          <button onClick={()=>window.location.href=`${API_URL}/strava/login/${authUser.id}`} style={{background:T.stravaLight,border:`1px solid ${T.strava}`,borderRadius:8,padding:"6px 10px",color:T.strava,fontSize:12,fontWeight:600,cursor:"pointer"}}>Reconnect</button>
          <button onClick={onDisconnect} style={{background:T.redLight,border:`1px solid ${T.red}`,borderRadius:8,padding:"6px 10px",color:T.red,fontSize:12,fontWeight:600,cursor:"pointer"}}>Disconnect</button>
        </div>
      </div>
      <div style={{display:"flex",gap:4,marginBottom:16,background:T.bg,borderRadius:8,padding:4}}>
        {[{id:"week",label:"This Week"},{id:"month",label:"This Month"},{id:"ytd",label:"Year to Date"},{id:"recent",label:"Recent"}].map(t=>(
          <button key={t.id} onClick={()=>setView(t.id)} style={{flex:1,padding:"6px 4px",borderRadius:6,border:"none",background:view===t.id?T.card:"transparent",color:view===t.id?T.strava:T.muted,fontSize:isMobile?10:12,fontWeight:view===t.id?700:400,cursor:"pointer",boxShadow:view===t.id?"0 1px 4px rgba(0,0,0,0.08)":"none"}}>{t.label}</button>
        ))}
      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {d.recent.map((act,i)=>(
            <div key={i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 12px",background:T.stravaLight,borderRadius:10}}>
              <div style={{width:36,height:36,borderRadius:"50%",background:T.strava+"20",display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}><ActivityTypeIcon type={act.type}/></div>
...
function AuthPage({ onLogin }) {
  ...
  const [mode,setMode]=useState("login"),[email,setEmail]=useState(""),[username,setUsername]=useState(""),[password,setPassword]=useState(""),[busy,setBusy]=useState(false),[err,setErr]=useState("");
  const submit=async()=>{
      ...
      <div style={{flex:1,background:T.bg,display:"flex",alignItems:"center",justifyContent:"center",padding:isMobile?"32px 20px":48}}>
        <div style={{width:"100%",maxWidth:380}}>
          ...
          <div style={{marginBottom:24}}>
            <div style={{fontSize:isMobile?22:26,fontWeight:700,color:T.text,marginBottom:6,fontFamily:"'Nunito Sans',system-ui"}}>{mode==="login"?"Welcome back":"Create account"}</div>
          ...
          <div style={{display:"flex",background:T.border+"50",borderRadius:10,padding:4,marginBottom:24}}>
            {["login","register"].map(m=>(
              <button key={m} onClick={()=>{setMode(m);setErr("");}} style={{flex:1,padding:"9px",borderRadius:8,border:"none",background:mode===m?T.card:"transparent",color:mode===m?T.text:T.muted,fontSize:13,fontWeight:mode===m?600:400,cursor:"pointer",transition:"all 0.15s",boxShadow:mode===m?"0 1px 4px rgba(0,0,0,0.08)":"none"}}>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    if(savedToken&&savedUser){
      ...
      setToken(savedToken);setAuthUser(parsedUser);
  ...
  const checkIntegrations=async(uid)=>{
    try{
      const [fitRes,strRes]=await Promise.all([fetch(`${API_URL}/fitbit/status/${uid}`),fetch(`${API_URL}/strava/status/${uid}`)]);
            ...
            <div style={{flex:1,padding:"12px 8px",overflowY:"auto"}}>
              ...
              {navItems.map(n=>(
                <NavItem key={n.id} icon={n.icon} label={sidebarOpen?n.label:""} active={tab===n.id}
                  onClick={()=>{if(n.id==="recovery")fetchRecovery();else if(n.id==="goal")fetchGoal();else setTab(n.id);}}/>
              ))}
                    ...
                    <Card accent={fitbitConnected?T.green:T.border} style={{marginBottom:isMobile?12:16}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}>
                        ...
                        <div style={{display:"flex",gap:6,flexShrink:0,flexWrap:"wrap"}}>
                          ...
                          {fitbitConnected&&<button onClick={disconnectFitbit} style={{background:T.redLight,border:`1px solid ${T.red}`,borderRadius:8,padding:"7px 10px",color:T.red,fontSize:12,fontWeight:600,cursor:"pointer"}}>Disconnect</button>}
                        </div>
                      </div>
                    </Card>

                    {/* Strava Dashboard */}
                    <StravaDashboard authUser={authUser} stravaConnected={stravaConnected} stravaData={stravaData} stravaSyncing={stravaSyncing} onSync={()=>syncStrava(authUser.id)} onDisconnect={disconnectStrava} isMobile={isMobile}/>
                    ...
                    {!recovery?(
                      ...
                      <>
                        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16,flexWrap:"wrap"}}>
                          <div style={{fontSize:18,fontWeight:700,color:T.text}}>Recovery Plan</div>
                          {recovery.source&&<LiveBadge source={recovery.source}/>}
                        ...
                        <Card accent={T.green}>
                          <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                            ...
                            <div>
                              ...
                              <div style={{fontSize:12,color:T.muted,lineHeight:1.6}}>{recovery.note||"Prioritise sleep quality and avoid high-intensity sessions until day 7. Stay hydrated and listen to your body."}</div>
                            </div>
                          </div>
                        </Card>
                      </>
                ...
                {tab==="goal"&&(
                  <div className="fade">
                    {!goal?(
                      ...
                      <>
                        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16,flexWrap:"wrap"}}>
                          <div style={{fontSize:18,fontWeight:700,color:T.text}}>AI Goal</div>
                          {goal.source&&<LiveBadge source={goal.source}/>}
                          {goal.risk&&<Pill color={riskColor(goal.risk)} bg={riskColor(goal.risk)+"18"}>{goal.risk} Risk</Pill>}
                        </div>
                        <Card accent={T.green} style={{marginBottom:isMobile?14:20}}>
                          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
                            <div>
                              ...
                              {goal.duration&&<div style={{fontSize:12,color:T.muted,marginTop:2}}>Duration: {goal.duration}</div>}
                            </div>
